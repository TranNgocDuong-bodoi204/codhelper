const T1_RAW = 10345;
const T1 = T1_RAW * 1.45;
const T1_sgl = 5;
const T1_GH = 1;
const T2_RAW = 20690;
const T2 = T2_RAW * 1.45;
const T2_sgl = 10;
const T2_GH = 2;
const T3_RAW = 41380;
const T3 = T3_RAW * 1.45;
const T3_sgl = 20;
const T3_GH = 3;
const T4_RAW = 55173;
const T4 = T4_RAW * 1.45;
const T4_sgl = 40;
const T4_GH = 4;
const T5_RAW = 82759;
const T5 = T5_RAW * 1.45;
const T5_sgl = 100;
const T5_GH = 10;
...
const T1_T4_sgl = 35;
const T1_T4_GH = 3;
const T2_T4 = 50000.35;
const T2_T4_sgl = 30;
const T2_T4_GH = 2;
const T3_T4 = 19999.85;
const T3_T4_sgl = 20;
const T3_T4_GH = 1;
const T1_T5 = 105000.3;
const T1_T5_sgl = 95;
const T1_T5_GH = 9;
const T2_T5 = 90000.05;
const T2_T5_sgl = 90;
const T2_T5_GH = 8;
const T3_T5 = 59999.55;
const T3_T5_sgl = 80;
const T3_T5_GH = 7;
const T4_T5 = 39999.7;
const T4_T5_sgl = 60;
...
function CalculateQuantityTroops() {
  const days = NumberUnFormat(document.getElementById("daysNumber").value);
  const hours = NumberUnFormat(document.getElementById("hoursNumber").value);
  const minutes = NumberUnFormat(document.getElementById("minutesNumber").value);
  const buff = ReadNumberInput("buff") / 100 || 0;
  ...
  const troopResults = [
    ...
    ["t4", quantityT4, "T4"],
    ["t5", quantityT5, "T5"],
    ["t1T4", quantityT1_T4, "T1_T4"],
    ["t2T4", quantityT2_T4, "T2_T4"],
    ["t3T4", quantityT3_T4, "T3_T4"],
    ["t1T5", quantityT1_T5, "T1_T5"],
    ["t2T5", quantityT2_T5, "T2_T5"],
    ["t3T5", quantityT3_T5, "T3_T5"],
    ["t4T5", quantityT4_T5, "T4_T5"],
  ];

  troopResults.forEach(([id, quantity, tier]) => {
    document.getElementById(`${id}SglP`).textContent = GetEventPoints(
      quantity,
      tier,
      "strongest lord",
    );
    document.getElementById(`${id}GhP`).textContent = GetEventPoints(
      quantity,
      tier,
      "greate hight",
    );
...
function GetEventPoints(quantity, tier, event) {
  ...
  const pointsPerTroop =
    pointTable[String(event).trim().toLowerCase()]?.[
      String(tier).trim().toUpperCase()
    ];
  const troopQuantity = Number(
    String(quantity).replace(/\./g, "").replace(/,/g, ""),
  );
...
function CalculateQuantity(totalDays, baseline, buff) {
  return Math.round(
    (totalDays / (baseline / (1 + buff))) * 86400 * 1000,
  ).toLocaleString("vi-VN");
}

function CalculateSpeed() {
  const result = document.getElementById("resultSpeed");
  const numberOfTroops = NumberUnFormat(document.getElementById("quantityTroops").value);
  const buff = (ReadNumberInput("buff") || 0) / 100;
  const baseline =
    parseFloat(document.getElementById("selectTroopType").value) || 0;
  ...
  daysI.value = NumberFormat(wholeDays);
  hoursI.value = NumberFormat(hours);
  minutesI.value = NumberFormat(minutes);
...
function CalculateEventTraining() {
  ...
  const quantity = NumberUnFormat(document.getElementById("eventTroopQuantity").value);
  ...
  if (
    ...
    eventResult.innerHTML =
      "<p>Vui lòng nhập ngày sự kiện và số lượng lính hợp lệ.</p>";
    return;
  }
  ...
  const idealStart = new Date(
    eventStart.getTime() - requestedTrainingSeconds * 1000,
  );
...
function StartNumberFormatting() {
  document.querySelectorAll(".formatted-number").forEach((input) => {
    input.addEventListener("input", () => {
      const rawValue = input.value.replace(/[^\d]/g, "");
      input.value = rawValue ? NumberFormat(Number(rawValue)) : "";
    });
  });
}
...
function OnRadioSelector(select) {
  ...
  init_totalTroops = NumberUnFormat(
    document.getElementById(quantityId).textContent,
  );
  totalTroopRemain = init_totalTroops;
  troop_tier_rss = select;

  RSS_Cal_Element.totalTroop.textContent = NumberFormat(init_totalTroops);
  ResetTroopInputs()
  CalculateRSSValue();
}
...
function onTroopInputChange(input) {
  ...
  input.value = value ? NumberFormat(value) : "";
...
function SelectAllRemainingTroops(inputId) {
  ...
  const available = Math.max(
    0,
    NumberUnFormat(init_totalTroops) - otherInputsTotal,
  );
  input.value = NumberFormat(currentValue + available);
...
function CalculateRSSValue() {
  ...
  document.getElementById("totalGold").textContent = NumberFormat(
    rss_value.gold,
  );
  document.getElementById("totalWood").textContent = NumberFormat(
    rss_value.wood,
  );
  document.getElementById("totalStone").textContent = NumberFormat(
    rss_value.stone,
  );
  document.getElementById("totalMana").textContent = NumberFormat(
    rss_value.mana,
  );
}
...
function NumberFormat(params) {
  return Number(params || 0).toLocaleString("vi-VN");
}
